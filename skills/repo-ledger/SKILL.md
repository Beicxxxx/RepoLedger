...
1. Before referencing an ID, run `repo-ledger lookup <ID> --json`. The argument may be a current ID or a registered `legacy` alias; if it is old, resolve it to the returned current ID before writing. Before creating an entity, inspect relevant existing references and query plausible matches; lightweight title search is not implemented yet.
   - Every natural-language reference must include the full registered name, even when the input is already a bare `TASK-36` or `ISSUE-30`; bare IDs are allowed only in CLI arguments, registry fields, JSON fields, or other machine-facing positions. Normalize shorthand such as `task36` or `issue30` to `TASK-36` or `ISSUE-30` for lookup, then write `TASK-36 主实验 2×2` or `ISSUE-30 去提示渲染后自身距离未达约 0`. Keep surrounding explanation in the user's language, preserve the registry's canonical name, and never output bare shorthand or invent a name when lookup fails.
