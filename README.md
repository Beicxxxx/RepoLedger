...
- 页面中每处 ID 后都紧跟完整登记名称：note 与类型 `description` 中出现的已登记 ID 若后面没有名称，生成时补上；未登记的 token 原样保留，登记名称本身逐字输出。路径（带扩展名的文件名、目录名之后）、URL 和反引号中的 ID 不补名，以免改掉路径或代码；只用 `/` 连接的几个 ID 仍照常补名。
- 登记文本只对会触发 Markdown 或 HTML 语法的字符加反斜杠转义（例如成对星号、反引号、尖括号标签、词边界下划线、任意位数的数字字符引用）；普通文字、中文和词内下划线逐字保留，因此原文里“ID + 名称”的写法不被转义打断。裸 URL 原样输出而不逐字符转义：带协议的写成 `<…>` 自动链接，`www.` 开头或含 `%` 转义、`xn--`、字符引用的写成行内代码，表格中的 `|` 写作 `\|`；GitHub（GFM）与 CommonMark 渲染出的文字都与登记原文一致。
...
- `--out` 相对当前目录解析，优先于 `[index] out_dir`（相对项目根）；两者都没有时以 `ERR_INDEX_OUT_DIR` 退出码 `2` 结束。输出目录解析后（跟随符号链接与 `..`）必须在项目根之内；`[index] out_dir` 本身或其上级目录是符号链接时，`index` 与 `check` 都以 `ERR_INDEX_OUT_DIR` 退出码 `2` 结束。`index` 从不删除文件，拒绝覆盖首行不是生成标记的文件，也拒绝把项目外的目录、项目根、`.git` 内部或账本所在目录（无论账本扩展名）用作输出目录，同样以 `ERR_INDEX_OUT_DIR` 退出码 `2` 结束；多余页面只在输出里列出。
