...
- 默认 `order` 必须等于 ID 序号。设置 `independent_order = true` 后，`order` 成为同一类型、同一 parent 下的计划位置：可被 `update --order` 重排，只影响 `tree` 的渲染顺序，不参与身份、状态或完成判断。此时计划位置允许并列（真实账本常出现），需要严格唯一时再设 `unique_order_within_scope = true`。
