...
- `--out` 相对当前目录解析，优先于 `[index] out_dir`（相对项目根）；两者都没有时以 `ERR_INDEX_OUT_DIR` 退出码 `2` 结束。输出目录解析后（跟随符号链接与 `..`）必须在项目根之内。`index` 从不删除文件，拒绝覆盖首行不是生成标记的文件，也拒绝把项目外的目录、项目根、`.git` 内部或账本所在目录（无论账本扩展名）用作输出目录，同样以 `ERR_INDEX_OUT_DIR` 退出码 `2` 结束；多余页面只在输出里列出。
