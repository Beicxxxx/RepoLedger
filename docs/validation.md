...
未验证：Windows 与 macOS、GitHub 网页上的实际渲染。试生成没有写入原仓库。

项目自身守卫对生成页的反应，由独立评审在导出副本上实测：副本 `git init` 后把这 12 页加入受控清单，按 `scripts/verify_all.py` 的原参数运行 `scripts/check_no_legacy_codes.py`。不加页面时 0 处阻断；加入后 264 处阻断，退出码 1，阻断全部落在页面上。其中 245 处记号出现在 `legacy:` 行内代码里（有 108 处同时出现在 note 中），19 处只出现在 note 中。该守卫用 `--allow-file` 把账本本身降为告警，页面没有同样的待遇，所以只去掉 legacy 字段也不够。业主已决定在该守卫中豁免目录页的输出目录。评审实测的 12 页与返工后重新生成的页面逐字节相同。
