...
7. 全名规则：页面中每处已登记 ID 后都紧跟“空格 + 完整登记名称”。标题与关系字段直接写“ID + 名称”。note 与类型 `description` 属于自由文本：其中的 ID 若后面已经是“空格 + 完整名称”则保持原样，否则在 ID 后插入名称。识别用与引用扫描相同的 token 形状，但边界只看 ASCII 字母、数字、下划线和连字符，所以紧贴中文的 ID 也会补名。未登记的 token 原样保留；登记名称本身逐字输出，即使其中含有其他 ID。legacy 值写成行内代码，属于机器位置。
...
config：配置与 legacy_guard；registry：严格 codec、legacy 映射、分配与更新；search：有限候选检索；git：Git inventory 和路径约束；linter：关系/锚点/引用/退役代号守卫；index：按类型目录页的确定性渲染、新鲜度比较与 check 集成；cli：命令与诊断；errors：共享诊断结构。
