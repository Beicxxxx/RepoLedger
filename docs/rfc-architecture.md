...
`repo-ledger check` 聚合两条可选的正文守卫，`repo-ledger check-naming` 单独运行它们。二者默认关闭，逐行判定 Markdown 文本，先把代码围栏（``` 与 ~~~，含 legacy 教学围栏）和行内代码按列替换成空格；登记表本身始终单独解析、不扫描。实现与规则全文在 `repo_ledger/naming.py`，规则版本 `NAMING_RULES_VERSION = 2`（第 1 版的基线在棘轮里被跳过，不作根）。
