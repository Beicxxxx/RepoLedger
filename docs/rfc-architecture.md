...
输出目录：`--out` 相对当前目录解析，并优先于 `[index] out_dir`；解析时跟随符号链接与 `..`，结果必须位于项目根之内，因此页面里的相对链接不依赖项目所在的绝对路径。out_dir 相对项目根，经过与 registry_path 相同的安全检查，不接受结尾斜杠。两者都没有时返回 ERR_INDEX_OUT_DIR（配置错误，退出码 2）。输出目录不能在项目根之外，也不能是项目根、`.git` 内部或账本所在目录（与账本的扩展名无关），违者同样返回 ERR_INDEX_OUT_DIR。写入只替换内容有变化的页面；已存在但首行不是生成标记的文件一律拒绝覆盖；生成器从不删除文件，输出目录顶层多余的 `.md` 只在结果中列为 extra。
