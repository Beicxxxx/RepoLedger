...
输入与确定性：页面只取决于 ledger.toml 与账本文件字节。生成器只读一次账本，同一份字节既用于解析，也用于计算 sha256；每页首行是 `<!-- registry: <registry_path>; registry-sha256: <前 12 位>; generated by repo-ledger index; do not edit by hand -->`。不调用 Git、不读时钟、不依赖项目所在的绝对路径；输出为 UTF-8、LF，结尾恰好一个换行；类型按 ledger.toml 声明顺序，状态按词表顺序。导出的非 Git 副本同样可以生成。账本关系错误（目标不存在、自引用、环）时拒绝生成，不输出残缺页面。
