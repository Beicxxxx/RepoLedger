...
8. 转义：登记文本只转义会改变渲染的字符，包括后接 ASCII 标点或位于末尾的反斜杠、反引号、星号、波浪号、美元符、不在词内的下划线、后接字母或 `/!?` 的 `<`、构成 HTML 实体引用的 `&`、紧接 `(` 的 `]`、表格单元中的 `|`，以及标题末尾的 `#`。普通文字、中文、词内下划线和孤立的比较符逐字节保持不变，因此原文中“ID + 名称”的写法不会被转义打断。
